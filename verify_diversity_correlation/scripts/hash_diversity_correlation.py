...
def main():
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"  Spearman ρ: {stats['spearman_r']:.4f}\n")
        f.write(f"  Kendall τ: {stats['kendall_tau']:.4f}\n")
        if not np.isnan(stats['distance_corr']):
            f.write(f"  Distance corr: {stats['distance_corr']:.4f}\n")
        if not np.isnan(stats['mic']):
            f.write(f"  MIC: {stats['mic']:.4f}\n")
