# This script will run over all the cluster CSV files, pick a representative for each cluster, and output a CSV file
# of all the individual communities. Next, it will then perform hierarchical clustering of these representatives. Then,
# it will create a distance matrix using the query_pc_mat C++ program, then, since the Jaccard of each cluster is
# all pairwise ones, I will take the distances between representative sample S_1 and all of it's Jaccard=1 neighbors
# s_j, and expand the clustered distance matrix to endown the s_j with the same distances of J(S_1, \cdot). Then,
# I will use the seaborn clustermap to plot this.
# !/usr/bin/env python3
"""
...
def find_communities(adjacency: Dict[str, Set[str]], logger: logging.Logger) -> Dict[int, List[str]]:
    """
    Find connected components (communities) in the adjacency graph.
    ...
    def dfs(node: str, current_community: List[str]) -> None:
